...
file = 'C:/Python33/HW/restaurants_small.txt'
...
def build_rating_list(name_to_rating, names_final):
    ...
    for name in names_final:
        result.append (name_to_rating[name])
        result.append(name)
...
def filter_by_cuisine(names_matching_price, cuisine_to_names, cuisines_list):
    ...
    >>> cuis = {'Canadian': ['Georgie Porgie'],   ## cuisine_to_names
     'Pub Food': ['Georgie Porgie', 'Deep Fried Everything'],
     'Malaysian': ['Queen St. Cafe'],
     'Thai': ['zzz','Queen St. Cafe'],
     'Chinese': ['Dumplings R Us'],
     'Mexican': ['Mexican Grill']}
    ...
    restaurants = []
    names_final = []
    for food in cuisines_list:
        if food in cuisine_to_names:
            restaurants.extend(cuisine_to_names[food])
...
def read_restaurants(file):
    ...
    #restaurants = askopenfile(mode='r', title='Select word list file')
    restaurants = open(file, 'r')
    #restaurants = open(file,'r')
    clean_list = []
    z = restaurants.readlines()
    for item in z:
        if item != "\n":
            clean_list.append(item[:-1])

    i = 0
    for i in range (0,len(clean_list),4):
        name_to_rating [clean_list[i]] = clean_list[i+1]
        
        price_to_names [clean_list[i+2]].append (clean_list[i])    

        cuizine_split = (clean_list[i+3]).split(',')
        for item in cuizine_split:
            if item not in cuisine_to_names:
                cuisine_to_names [item] = [clean_list[i]]
            else:
                cuisine_to_names [item].append (clean_list[i])
